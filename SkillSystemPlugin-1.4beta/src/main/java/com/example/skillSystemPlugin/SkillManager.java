...
public class SkillManager {
    ...
    public void addXP(Player player, SkillType skill, int amount) {
        ...
        player.spigot().sendMessage(ChatMessageType.ACTION_BAR,
                new TextComponent("§eSkill §6" + skill.name() + ": §f" + currentXP + "/" + xpToLevel));

        if (levelsGained > 0) {
            player.sendMessage("§aSkill §e" + skill.name() + " §aup in §e" + levelsGained +
                    " §alevels! New level: §e" + currentLevel);
        }
    }
